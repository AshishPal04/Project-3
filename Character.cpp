...
{
    if ((getName() == rhs.getName()) && (getRace() == rhs.getRace()) && (getLevel() == rhs.getLevel()) && (isEnemy() == rhs.isEnemy()))
    {
        return true;
    }
    return false;
...
string Character::display() const
{
    string is_enemy = (enemy_ ? "an enemy" : "not an enemy");
    // cout<<getName()<<" is a Level "<<getLevel()<<" "<<getRace()<<"."<<endl;
    // cout<<"Vitality: "<<getVitality()<<endl; 
    // cout<<"Armor: "<<getArmor()<<endl; 
    // cout<<"They are "<<(isEnemy() ? "an enemy." : " not an enemy.")<<endl;
    return "[" + name_ + "] is a Level " + to_string(level_) + " " + getRace() + ". \nVitality: " + to_string(vitality_) + "\nMax Armor: " + to_string(armor_) + "\nThey are " + is_enemy + ".\n";
